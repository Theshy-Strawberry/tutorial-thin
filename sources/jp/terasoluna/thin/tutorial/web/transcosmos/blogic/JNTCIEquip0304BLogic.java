...
public class JNTCIEquip0304BLogic implements BLogic<JNTCIEquip03ListInput> {
	...
	public BLogicResult execute(JNTCIEquip03ListInput param) {
		...
		if ("1".equals(param.getHidden_state())) {
			updateDAO.execute("updateEquipapplyRepair0303", param);
			updateDAO.execute("updateEquipapplyRepair03031", param);
			updateDAO.execute("updateEquipapplyRepair03032", param);
			...
			String content = "nameさん<br>お疲れ様です。<br><br>设备修理失败 .<br><br>以上。";
			...
			updateDAO.execute("updateEquipapplyRepair03033", param);
			updateDAO.execute("updateEquipapplyRepair03034", param);
			updateDAO.execute("updateEquip0303", param);
			String offic_mail = queryDAO.executeForObject("sendmailperson",
					param, String.class);
			String address = offic_mail + "@trans-cosmos.com.cn";
			InternetAddress sendTo[] = new InternetAddress[1];
			...
			String content = "nameさん<br>お疲れ様です。<br><br>设备修理成功 .<br><br>以上。";
			content = content.replaceAll("name", param.getApply_user_name()
					.substring(0, 1));
