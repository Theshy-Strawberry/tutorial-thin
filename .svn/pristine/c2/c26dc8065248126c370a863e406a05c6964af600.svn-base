...
public class JNTCIPersonalInfoInitBLogic implements BLogic<JNTCIPersonalInfoLoginInput> {
    ...
    public BLogicResult execute(JNTCIPersonalInfoLoginInput param) {
        ...
        if(jn_card != null && !("".equals(jn_card.trim()) )){
        	//鍙栧緱涓汉鍩烘湰淇℃伅
        	JNTCIPersonalInfoLoginOutput personalInfoLoginOutput = queryDao.executeForObject("selectUpdatePersonalInfo4", param, JNTCIPersonalInfoLoginOutput.class);
        	...
        	if(personalInfoLoginOutput2!=null){
            	if(!"".equals(personalInfoLoginOutput2.getLeader())&&personalInfoLoginOutput2.getLeader()!=null){
            		personalInfoLoginOutput.setLeader(personalInfoLoginOutput2.getLeader());
            	}
            	if(!"".equals(personalInfoLoginOutput2.getDeptPerson())&&personalInfoLoginOutput2.getDeptPerson()!=null){
			...
			//personalInfoLoginOutput.setId_Card(jn_card);
			/*personalInfoLoginOutput.setDepartment1(department);
			personalInfoLoginOutput.setUserName1(userName);*/
			personalInfoLoginOutput.setStartIndex(startIndex);
			personalInfoLoginOutput.setCardnum(cardnum);
			personalInfoLoginOutput.setCardnum(cardnum);
			personalInfoLoginOutput.setEntryTime_Begin(entryTime_Begin);
			personalInfoLoginOutput.setEntryTime_End(entryTime_End);
