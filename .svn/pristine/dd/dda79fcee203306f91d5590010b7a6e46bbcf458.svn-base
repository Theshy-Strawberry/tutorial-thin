...
public class JNTCIEmpInitBlogic implements BLogic<JNTCIEmpPositionInput>{
	...
	public BLogicResult execute(JNTCIEmpPositionInput arg0) {
		...
		// 一覧の従業員情報
		List<JNTCIEmpPositionVo> emplist = queryDAO.executeForObjectList("InitEmpInfo",null );
		// 一覧の従業員情報のCOUNT
		int  count = emplist.size();
		// プルダウンリストに従業員情報
		List<JNTCIEmpPositionVo> infoEmpNoList = queryDAO.executeForObjectList("infoEmpNoListId",null );
