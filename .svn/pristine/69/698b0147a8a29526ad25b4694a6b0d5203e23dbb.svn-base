...
public class JNTCIAddsignInfoQueryRefuseBLogic implements
    ...
    public BLogicResult execute(JNTCIAddsignInfoInput InfoQueryDelete) {
    	...
    	InfoQueryDelete.setExamine_status("A05");
                		...
                		if((ShenseiM==SM&&ShenseiY==SY)||(dayCount<4&&ShenseiMNext==SM&&ShenseiYNext==SY)){
                			if("A06".equals(status)){
                        		if ("午前修正".equals(vo.getRETROACTIVE_TYPE())) {
                					...
                					InfoQueryDelete.setKojyo_time(0);
                				...
                				if(!("1070".equals(sysUser)||"1003".equals(sysUser)||"1004".equals(sysUser))){
                					InfoQueryDelete.setExamine_status("A07");
            					}
                				updateDAO.execute("statusUpdate", InfoQueryDelete);
                			}
                		}
