...
public class JNTCIAttendanceUpdate03Blogic implements
		BLogic<JNTCIAttendance03Dto> {
	...
	public BLogicResult execute(JNTCIAttendance03Dto param) {
		...
		// 获取数据库中的节假日列表
		List<JNTCIAttendance03Dto> detailDateList = queryDAO
				.executeForObjectList("detailDate", null);
		int i = 0;
		...
		if ("preservation".equals(param.getPreserVation())
				|| "preservation" == param.getPreserVation()) {
			BLogicMessages messages1 = new BLogicMessages();
			// 如果前台休假类型为婚假
			if ("AL3".equals(param.getLeaveType())) {
				// 查询数据库中婚假的总时间以及总次数
				JNTCIAttendance03Dto attendance03Dto1 = queryDAO
						.executeForObject("leaveTypeTime", JN_CARD,
				...
				Double LeaveTotalTime = Double.parseDouble(param
						.getLeaveTotalTime());
				// 获取数据库中leaveId的leaveType内容
				JNTCIAttendance03Dto leaveType = queryDAO.executeForObject(
						"searchTime", param, JNTCIAttendance03Dto.class);
				// 如果数据库中婚假次数为0次
				if (countNum == 0) {
					if (LeaveTotalTime > 24) {
						messages1.add("message", new BLogicMessage(
								"errors.leaveTypeTime.error"));
						attendance = queryDAO.executeForObject("leave",
								JN_CARD, JNTCIAttendance03Dto.class);
						if (attendance.deptId == null
								&& attendance.leaveId == null) {
							JNTCIAttendance03Dto attendance03Dto = queryDAO
									.executeForObject("selectdeptID", JN_CARD,
											JNTCIAttendance03Dto.class);
							attendance.setDeptId(attendance03Dto.getDeptId());
							attendance.setDeptName(attendance03Dto
									.getDeptName());
						}
						List<JNTCIAttendance03Dto> leaveTypeList = queryDAO
								.executeForObjectList("leaveType", 1);
						// 審査人（List）
						attendance.setDetailDate(aaString);
					...
					if ("AL3".equals(leaveType.getLeaveType())) {
						if (LeaveTotalTime2 > 24) {
							...
							if (attendance.deptId == null
									&& attendance.leaveId == null) {
								JNTCIAttendance03Dto attendance03Dto = queryDAO
										.executeForObject("selectdeptID",
												JN_CARD,
												JNTCIAttendance03Dto.class);
								attendance.setDeptId(attendance03Dto
										.getDeptId());
								attendance.setDeptName(attendance03Dto
										.getDeptName());
							}
						...
						Double leaveTypeTime1 = Double
								.parseDouble(leaveTypeTime);
				...
				JNTCIAttendance03Dto leaveTotalTime = queryDAO
						.executeForObject("searchTime", param,
								JNTCIAttendance03Dto.class);
				// 查询数据库中的leaveTotalTime1
				Double leaveTotalTime1 = Double.parseDouble(leaveTotalTime
						.getLeaveTotalTime());

				// 如果前台获取的休假种类是婚假
				if ("AL3".equals(leaveType.getLeaveType())) {
					// 查询数据库中的婚假总时间
					Double leaveTypeTime1 = Double.parseDouble(attendance03Dto1
							.getLeaveTypeTime());
					// 计算前台跟后台数据的和
					Double sumTime = leaveTypeTime1 - leaveTotalTime1
							+ LeaveTotalTime2;
					// 判断计算的和是否大于24小时
					if (sumTime > 24) {
						messages1.add("message", new BLogicMessage(
			...
			if ("AL3".equals(param.getLeaveType())) {
				...
				if (countNum == 0) {
					if (LeaveTotalTime > 24) {
						...
						List<JNTCIAttendance03Dto> leaveTypeList1 = queryDAO
								.executeForObjectList("leaveType", 1);
						// 審査人（List）
						attendance.setDetailDate(aaString);
			...
			// 年休假期申请的判断
			/*
			 * if(param.getLeaveType().equals("AL10")){ // 年休の余剰日数 Double
			 * remainDays = queryDAO.executeForObject("remainDays", JN_CARD,
			 * Double.class); // if(remainDays == null){ remainDays = 0.0; }
			 * double LeaveTotalTime =
			 * Double.parseDouble(param.getLeaveTotalTime()); double remainder =
			 * remainDays - LeaveTotalTime; if(remainder <= 0.0){ attendance =
			 * queryDAO.executeForObject("leave", JN_CARD,
			 * JNTCIAttendance03Dto.class); if (attendance.deptId != null &&
			 * attendance.leaveId != null) {} else{ JNTCIAttendance03Dto
			 * attendance03Dto = queryDAO.executeForObject("selectdeptID",
			 * JN_CARD, JNTCIAttendance03Dto.class);
			 * attendance.setDetailDate(aaString);
			 * attendance.setDeptId(attendance03Dto.getDeptId());
			 * attendance.setDeptName(attendance03Dto.getDeptName());
			 *
			 * } // 休むをタイプ(List) leaveTypeList=
			 * queryDAO.executeForObjectList("leaveType", null);
			 *
			 * attendance.setLeaveTypeList(leaveTypeList);
			 *
			 * examinePersonList= queryDAO.executeForObjectList("examinePerson",
			 * null); attendance.setDetailDate(aaString);
			 ...
			 * BLogicMessage("errors.leaveLeaveOff.error"));
			 * result.setResultObject(attendance); result.setMessages(messages);
			 * return result; }else{ param.setRemainDays(remainder); //
			 * 获取年假并时长减去，拿取减去的值更改剩余年假天数 updateDAO.execute("UpdateRemainDays",
			 * param); } }
			 */
			...
			// 获取表中的最大LEAVE_ID+1
			Integer LEAVE_ID = queryDAO.executeForObject("maxId", 1,
					Integer.class);
			...
			updateDAO.execute("JNTCIAttendance03Insert", param);
			String subject = "【系统邮件】【请假申请】 " + jnCard + "." + name + "." + leaveStartTime
					+ "—" + leaveEndTime;
			String content = pmName + "桑<br><br>工作辛苦了<br><br>" + reason
					+ "<br><br>以上";
			//代玉珍邮箱号
			 ...
			 *
			 * if (pmEmail != null && (!"".equals(pmEmail))) { InputStream in =
			 * null; try { in = new FileInputStream(filePath +pmEmail);
			 * props.load(in); } catch (FileNotFoundException e) {
			...
			Mail.sendAll(myEmail, "123456", pmEmails, daiyuzhenMail, null, subject, content,
					null);
			// 申请请假审核
			JNTCIAttendance03AuditStatusDto auditStatus1 = new JNTCIAttendance03AuditStatusDto();

			String EXAMINE_ID = queryDAO.executeForObject("eaxMaxID", 1,
					String.class);
			if (EXAMINE_ID == null) {
				EXAMINE_ID = "1";
			}
		...
		attendance = queryDAO.executeForObject("leave", JN_CARD,
				JNTCIAttendance03Dto.class);
		if (attendance.deptId == null && attendance.leaveId == null) {
			JNTCIAttendance03Dto attendance03Dto = queryDAO.executeForObject(
					"selectdeptID", JN_CARD, JNTCIAttendance03Dto.class);
			attendance.setDeptId(attendance03Dto.getDeptId());
			attendance.setDeptName(attendance03Dto.getDeptName());
		}
		// 休むをタイプ(List)
		...
		BLogicMessages messages = new BLogicMessages();
		messages.add("message", new BLogicMessage(
				"message.leaveInputInsert.success"));
